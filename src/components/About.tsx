...
const About = () => {
  const values = ["Integridade Ecológica", "Liderança em Sustentabilidade", "Inovação e Expertise", "Parceria com Clientes"];
  ...
  const autoplayPlugin = React.useRef(Autoplay({
    delay: 6000,
    // 6 seconds between slides (slower)
    stopOnInteraction: false,
    // don't pause on user interaction
    stopOnMouseEnter: false,
    // don't pause on mouse hover
    jump: false // prevents jumping between slides - smoother transitions
  }));
  return <section id="about" className="py-24 bg-secondary/70 relative overflow-hidden">
      {/* Decorative Elements */}
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          <div className="order-2 lg:order-1">
            <span className="inline-block bg-eco-50 px-3 py-1 rounded-full text-sm font-medium mb-4 opacity-0 animate-fade-in-right text-inherit">
              Sobre Nós
            </span>
            <h2 className="text-3xl md:text-4xl font-bold mb-6 opacity-0 animate-fade-in-right" style={{
            animationDelay: '0.1s'
          }}>
              Comprometidos com a Excelência Ambiental
            </h2>
            
            <p className="text-foreground/70 mb-8 opacity-0 animate-fade-in-right" style={{
            animationDelay: '0.2s'
          }}>
            ...
            <div className="space-y-4 mb-8 opacity-0 animate-fade-in-right" style={{
            animationDelay: '0.3s'
          }}>
            ...
            <a href="#contact" className="inline-block px-8 py-3 border border-eco-600 text-eco-600 rounded-full font-medium hover:bg-eco-600/10 transition-all duration-300 opacity-0 animate-fade-in-right" style={{
            animationDelay: '0.4s'
          }}>
              Saiba Mais
            </a>
          ...
          <div className="order-1 lg:order-2 opacity-0 animate-fade-in-left">
            ...
            loop: true,
            dragFree: true,
            // allows continuous dragging
            duration: 100 // slower transitions
          }} plugins={[autoplayPlugin.current]}>
              <CarouselContent>
                {plantingImages.map((image, index) => <CarouselItem key={index} className="md:basis-1/2 lg:basis-1/3">
                    <div className="p-1">
                      <div className="rounded-lg overflow-hidden shadow-xl">
                        <AspectRatio ratio={1 / 1}>
                          <img src={image} alt={`Crianças plantando ou cuidando de mudas de plantas - imagem ${index + 1}`} className="w-full h-full object-cover transition-transform duration-700 hover:scale-105" />
                        </AspectRatio>
                      </div>
                    </div>
                  </CarouselItem>)}
