...
const ServiceCard = ({
  ...
  return <div className={cn("glass rounded-lg p-8 transition-all duration-500 hover:shadow-lg hover:-translate-y-1 opacity-0 border-l-4 border-eco-600/50 hover:border-eco-700", index === 0 ? "animate-fade-in" : index === 1 ? "animate-fade-in-delay-1" : "animate-fade-in-delay-2")}>
...
const Services = () => {
  ...
  return <section id="services" className="py-24 relative overflow-hidden bg-gradient-to-b from-background via-eco-50/30 to-background">
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8 relative z-10">
        <div className="text-center mb-16">
          ...
          <h2 className="text-3xl md:text-4xl font-bold mb-6 opacity-0 animate-fade-in-delay-1 text-eco-700 text-center">Nossos Serviços</h2>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {services.map((service, index) => <ServiceCard key={service.title} title={service.title} description={service.description} icon={service.icon} index={index} />)}
        </div>
      </div>
